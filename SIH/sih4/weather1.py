...
CALENDAR_FILE = "crop_calendars_kerala_ml.csv" # Using the new file with Malayalam
...
def load_crop_calendars() -> pd.DataFrame:
    """Loads the crop calendar data from the CSV file."""
    try:
        df = pd.read_csv(CALENDAR_FILE)
        return df
    except FileNotFoundError:
        st.error(f"Error: The file '{CALENDAR_FILE}' was not found. Please make sure it's in the same folder as the script.")
        return pd.DataFrame()

# --- ANALYSIS ENGINE ---

class CropLifecycleAdvisor:
    """Analyzes weather against a crop's current life stage to generate prioritized alerts."""
    ...
    def get_stage_alerts(self, crop: str, planting_date: date, weather_data: Dict) -> List[Dict]:
        ...
        if status:
            triggered_alerts.insert(0, status)
        return triggered_alerts
        
    def _get_current_status(self, crop: str, plant_age: int) -> Optional[Dict]:
        crop_stages = self.calendars[self.calendars['crop'] == crop]
        for _, row in crop_stages.iterrows():
            if row['start_day'] <= plant_age <= row['end_day']:
                # Use .get() for safety; fall back to English if ML column doesn't exist
    ...
    def _check_weather_rule(self, rule: pd.Series, weather_data: Dict) -> Optional[Dict]:
...
# --- UI DISPLAY FUNCTIONS (WITH MALAYALAM RESTORED) ---
...
# --- MAIN APPLICATION ---
def main():
    st.set_page_config(page_title="Kerala Farmer AI", page_icon="🧑‍🌾", layout="wide")
    st.markdown("""<style>.weather-card { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 2rem; border-radius: 20px; color: white; text-align: center; margin: 1rem 0; box-shadow: 0 8px 32px rgba(0,0,0,0.1); } .temp-display { font-size: 4rem; font-weight: bold; margin: 1rem 0; } .weather-details { display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 1rem; margin-top: 1.5rem; } .detail-item { background: rgba(255,255,255,0.2); padding: 1rem; border-radius: 10px; } .detail-value { font-size: 1.5rem; font-weight: bold; display: block; } .forecast-card { background: white; border: 2px solid #e1e5e9; border-radius: 15px; padding: 1rem; text-align: center; height: 100%; } .forecast-date { font-weight: bold; color: #2c3e50; } .forecast-temp { font-size: 1.2rem; font-weight: bold; margin: 0.5rem 0; } .forecast-details { font-size: 0.9rem; }</style>""", unsafe_allow_html=True)
    st.title("🧑‍🌾 Krishi Sakhi / കേരള കർഷക AI ഉപദേശകൻ")
    crop_calendars = load_crop_calendars()
    if crop_calendars.empty: return
    ...
    st.markdown("---")
    if selected_district and selected_crop and planting_date:
